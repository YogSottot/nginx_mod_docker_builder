...
# zstd
docker cp "${id}":/usr/local/nginx/modules/ngx_http_zstd_filter_module.so fpm/mod_zstd/trixie/usr/lib/nginx/modules/ && \
docker cp "${id}":/usr/local/nginx/modules/ngx_http_zstd_static_module.so fpm/mod_zstd/trixie/usr/lib/nginx/modules/ && \
...
fpm \
  -s dir -t deb \
  -p libnginx-mod-http-zip_1.3.0-1.gbp8e65b82_trixie_amd64.deb \
  --name libnginx-mod-http-zip \
  --license bsd3 \
  --version 1.3.0-1.gbp8e65b82~trixie \
  --architecture amd64 \
  --depends "nginx-abi-1.26.3-3, libc6 (>= 2.4)" \
...
pushd fpm/mod_zstd/trixie/ && \
fpm \
  -s dir -t deb \
  -p libnginx-mod-http-zstd_0.1.1-1.gbp057a7d3_trixie_amd64.deb \
  --name libnginx-mod-http-zstd \
  --license bsd3 \
  --version 0.1.1-1.gbp057a7d3~trixie \
  --architecture amd64 \
  --depends "nginx-abi-1.26.3-3, libc6 (>= 2.4)" \
