...
# ntlm
docker cp "${id}":/usr/local/nginx/modules/ngx_http_upstream_ntlm_module.so ../fpm/mod_ntlm/jammy/usr/lib/nginx/modules/ && \
# delete image and container
docker rm -v "${id}" && \
# build libnginx-mod-http-ntlm deb package
pushd ../fpm/mod_ntlm/jammy/ && \
fpm \
  -s dir -t deb \
  -p libnginx-mod-http-ntlm_1.19.3-0.gbp3da77b0_jammy_amd64.deb \
  --name libnginx-mod-http-ntlm \
  --license bsd3 \
  --version 1.19.3-1.gbp3da77b0~jammy \
  --architecture amd64 \
