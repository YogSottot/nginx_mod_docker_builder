...
: "${MOD_NTLM_VERSION:?MOD_NTLM_VERSION is required}"
...
docker build \
  ...
  --build-arg "MOD_NTLM_VERSION=${MOD_NTLM_VERSION}" \
...
curl -fsSL \
  "https://raw.githubusercontent.com/gabihodoroaga/nginx-ntlm-module/${MOD_NTLM_VERSION}/LICENSE" \
  -o "${DOC_DIR}/LICENSE"
curl -fsSL \
  "https://raw.githubusercontent.com/gabihodoroaga/nginx-ntlm-module/${MOD_NTLM_VERSION}/README.md" \
  -o "${DOC_DIR}/README.md"
cp "${REPO_ROOT}/fpm/mod_ntlm/${VERSION_CODENAME}/debian/changelog.Debian" "${DEBIAN_DIR}/changelog.Debian"
