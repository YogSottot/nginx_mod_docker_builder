...
docker cp $id:/usr/local/nginx/modules/ngx_http_zip_module.so fpm/mod_zip/debian-12/usr/lib/nginx/modules/ && \
docker cp $id:/usr/local/nginx/modules/ngx_http_upstream_ntlm_module.so fpm/mod_ntlm/debian-12/usr/lib/nginx/modules/ && \
docker rm -v $id && \
docker image rm nginx_mod_docker_build && \
pushd fpm/mod_zip/debian-12/ && \
# build libnginx-mod-http-zip deb package
fpm \
  -s dir -t deb \
  -p libnginx-mod-http-zip_1.3.0+debian12~1.gbp8e65b82.deb \
  --name libnginx-mod-http-zip \
  --license bsd3 \
  --version 1.3.0+debian12~1.gbp8e65b82 \
  --architecture amd64 \
...
pushd fpm/mod_ntlm/debian-12/ && \
fpm \
  -s dir -t deb \
  -p libnginx-mod-http-ntlm_1.19.3+debian12~1.gbp3da77b0.deb \
  --name libnginx-mod-http-ntlm \
  --license bsd3 \
  --version 1.19.3+debian12~1.gbp3da77b0 \
  --architecture amd64 \
  --depends "nginx-abi-1.22.1-7, libc6 (>= 2.4)" \
